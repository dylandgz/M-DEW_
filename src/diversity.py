...
def _process_predictions(y, y_pred1, y_pred2):
    ...
    df['pred1'] = y_pred1
    df['pred2'] = y_pred2
    df['agreement'] = ~np.logical_xor(df['pred1'], df['pred2'])
    df['pred1_correct'] = ~np.logical_xor(df['pred1'], df['y_true'])
    df['pred2_correct'] = ~np.logical_xor(df['pred2'], df['y_true'])
    
    preprocess_df = pd.DataFrame()
    preprocess_df['00'] = np.logical_and(df['agreement'], ~df['pred1_correct']).astype(int)
    preprocess_df['01'] = np.logical_and(~df['agreement'], df['pred2_correct']).astype(int)
    preprocess_df['10'] = np.logical_and(~df['agreement'], df['pred1_correct']).astype(int)
    preprocess_df['11'] = np.logical_and(df['agreement'], df['pred1_correct']).astype(int)
    preprocess_df = preprocess_df.sum(axis=0) / len(preprocess_df)
    N00, N01, N10, N11 = tuple(preprocess_df)
